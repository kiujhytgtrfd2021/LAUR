...
class BertSelfAttention(nn.Module):
    def __init__(self, config,multi_params=None):
        ...
        if multi_params is not None:
            ...
            if config.bayes:
                self.query = BayesianLinear(config.hidden_size, self.all_head_size)
                self.key = BayesianLinear(config.hidden_size, self.all_head_size)
                self.value = BayesianLinear(config.hidden_size, self.all_head_size)
                self.bayes = True
            else :
    ...
    def forward(
        ...
        if self.bayes:
            mixed_query_layer = self.query(hidden_states,sample)
        else:
        ...
        if encoder_hidden_states is not None:
            if self.bayes:
                mixed_key_layer = self.key(encoder_hidden_states,sample)
                mixed_value_layer = self.value(encoder_hidden_states,sample)
            else:
            ...
            if self.bayes:
                mixed_key_layer = self.key(hidden_states,sample)
                mixed_value_layer = self.value(hidden_states,sample)
            else:
...
class BertSelfOutput(nn.Module):
    ...
    def forward(self, hidden_states, input_tensor,sample):
        if self.bayes :
            hidden_states = self.dense(hidden_states,sample)
        else:
...
class BertAttention(nn.Module):
    def __init__(self, config,multi_params=None):
        ...
        self.output = BertSelfOutput(config,multi_params)
    ...
    def forward(
        ...
        self_outputs = self.self(
            hidden_states, attention_mask, head_mask, encoder_hidden_states, encoder_attention_mask, output_attentions,sample
        )
        attention_output = self.output(self_outputs[0], hidden_states,sample)
...
class BertIntermediate(nn.Module):
    def __init__(self, config):
        super().__init__()
        if config.bayes:
            self.dense = BayesianLinear(config.hidden_size, config.intermediate_size)
            self.bayes = True
        else:
...
class BertOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
        if config.bayes:
            self.dense = BayesianLinear(config.intermediate_size, config.hidden_size)
            self.bayes = True
        else:
...
class BertLayer(nn.Module):
    ...
    def add_dataset(self, dataset):
        ...
        if dataset not in self.datasets:
            ...
            # self.multi_layers.append(BERTPRAM(self.config))
    ...
    def forward(
        ...
        if self.mult:
            ...
            # print(id(self.PRAM_layer.aug_dense.weight))
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        if config.mult:
            self.layer = nn.ModuleList([BertLayer(config,True) for _ in range(config.num_hidden_layers)])
            self.hidden_size,self.hidden_size_aug = config.hidden_size,config.hidden_size_aug
    ...
    def add_dataset(self, dataset):
        ...
        if dataset not in self.datasets:
            ...
            # print("add_dataset")
            # print(self.datasets)
            self.mult_aug_dense.append(nn.Linear(self.hidden_size, self.hidden_size_aug))
            self.mult_aug_dense2.append(nn.Linear(self.hidden_size_aug, self.hidden_size))
            for l, layer in enumerate(self.layer):
                layer.add_dataset(dataset)
    ...
    def set_dataset(self, dataset):
        ...
        for l, layer in enumerate(self.layer):
            ...
            # print(id(layer.PRAM_layer.aug_dense.weight))
...
class BertPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module,BayesianLinear):
            module.weight.mu.data.normal_(mean=0.0, std=self.config.initializer_range)
        elif isinstance(module, BertLayerNorm):
            module.bias.data.zero_()
            module.weight.data.fill_(1.0)
        if isinstance(module, BayesianLinear) and module.bias is not None:
            module.bias.data.zero_()
...
class BertModel(BertPreTrainedModel):
    ...
    Return:
        ...
        pooled_output = self.pooler(sequence_output,sample)
...
class BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        if config.bayes:
            self.classifier = BayesianLinear(config.hidden_size, config.num_labels)
        else:
...
class BertForMultipleChoice(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        if config.bayes:
            self.classifier = BayesianLinear(config.hidden_size, 1)
        else:
...
class BertForQuestionAnswering(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.bert = BertModel(config)
        if config.bayes:
            self.qa_outputs = BayesianLinear(config.hidden_size, config.num_labels)
        else:
