...
sys.path.append('/home/wsp1996/CSUR/WikiSQL/sqlova')
...
class Decoder_s2s(nn.Module):
    ...
    def forward(self, wenc_s2s, l_input, cls_vec, pnt_start_tok, g_pnt_idxs=None,):
        ...
        wenc_s2s = wenc_s2s.unsqueeze(1) #[B,1,270,768]
        # h_0 and c_0 from cls_vec
        # They are not bidirectional.
        h_0 = torch.zeros([self.lS, bS, self.hS]).to(device) #[2,B,100]
        ...
        if g_pnt_idxs:
            ...
            pnt_n = torch.zeros(bS, self.Tmax, mL_input).to(device)  # one hot [B,200,270]
            # assign index
            ...
            # Encode
            dec_pn, _ = self.decode_pn(pnt_n, (h_0, c_0)) #[B,200,100]
            dec_pn = dec_pn.contiguous()

            # [bS, T, iS]
            dec_pn = dec_pn.unsqueeze(2) #[B,200,1,100]

            # Calculate score
            s_wv = self.wv_out(
                self.W_s2s(wenc_s2s)  #[B,1,270,100]
                + self.W_pnt(dec_pn)    #[B,200,1,100]
