...
def custom_regularization(saver_net, trainer_net, mini_batch_size, loss=None):
    ...
    loss = loss + args.alpha * (mu_weight_reg_sum + mu_bias_reg_sum) / (2* mini_batch_size)
    # L1 loss
    loss = loss + args.beta * (L1_mu_weight_reg_sum + L1_mu_bias_reg_sum) / (mini_batch_size)
    # # sigma regularization
    loss = loss + args.gamma * (sigma_weight_reg_sum) / (mini_batch_size)
