...
class Trainer:
    ...
    def create_optimizer_and_scheduler(self, num_training_steps: int):
        ...
        if self.optimizer is None:
            ...
            params = get_params(self.model,self.args.learning_rate)
            if self.wo_rho == False and self.freeze == False:
                self.optimizer = AdamW_bayes(params,
                ...
                self.optimizer = AdamW(params,eps=1e-8)
            # self.optimizer = AdamW(params,eps=1e-8)
    ...
    def compute_loss(self, model, inputs):
        ...
        """
        outputs = model(**inputs,sample = True,use_class = True)

        loss = outputs[0]
        # print(self.args.train_batch_size)
        loss = custom_regularization(self.model_old,model,self.args.train_batch_size,loss)
    ...
    def prediction_step(
        ...
        with torch.no_grad():
            if self.use_amp:
                with autocast():
                    outputs = model(**inputs,sample = False,use_class = True)
            else:
                outputs = model(**inputs,sample = False,use_class = True)
            if has_labels:
