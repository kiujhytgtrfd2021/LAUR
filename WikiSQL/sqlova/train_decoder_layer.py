...
from LAUR_model.model import BertClassifier_LAUR,BertClassifier_base
...
def construct_hyper_param(parser):
    ...
    parser.add_argument("--old_model_path", default='', type=str)

    parser.add_argument("--use_regular",default=False,action='store_true')

    parser.add_argument("--do_train",default=False,action='store_true')

    parser.add_argument("--no_regular",default=False,action='store_true')
...
def get_opt(model, model_bert, model_type):
    rho_id,mu_id = [],[]
    for n,param in model_bert.named_parameters():
        if 'rho' in n:
            rho_id.append(id(param))
        if 'weight_mu' in n:
            mu_id.append(id(param))
    params = get_params(model_bert,args.lr_bert)
    if args.worho == False and args.freeze == False:
        opt_bert = AdamW_bayes(params,
        ...
        opt_bert = AdamW(params,eps=1e-8)
    # opt=opt_bert
...
def get_models(args, BERT_PT_PATH, trained=False, path_model_bert=None, path_model=None):
    ...
    model_bert = BertClassifier_LAUR()
    if args.no_regular:
        model_bert = BertClassifier_base()
    model_bert.add_dataset("WikiSQL")
    model_bert.set_dataset("WikiSQL")

    model_bert.cuda()
    tokenizer, bert_config = BertTokenizer.from_pretrained('bert-base-uncased', do_lower_case=True),BertConfig()
    args.iS = bert_config.hidden_size * args.num_target_layers  # Seq-to-SQL input vector dimenstion


    model = model_bert.SQL_outputs
    # Get Seq-to-SQL
...
def train(train_loader, train_table, model, model_bert, opt,scheduler, tokenizer,sql_vocab,
          max_seq_length, accumulate_gradients=1, check_grad=False,
          st_pos=0, opt_bert=None,bert_scheduler=None, path_db=None, dset_name='train', col_pool_type='start_tok', aug=False):
    model.train()
    model_bert.train()
...
if __name__ == '__main__':
    ...
    ## 2. Paths
    path_h = 'data'
    path_wikisql = os.path.join(path_h, 'WikiSQL')
    BERT_PT_PATH = path_h
    ...
    if args.load_model:
        model_bert = torch.load(args.old_model_path)
        model_bert.add_dataset("WikiSQL")
        model_bert.set_dataset("WikiSQL")
        model_bert.cuda()
        model = model_bert.SQL_outputs
    ...
    total_steps = len(train_loader) * args.tepoch
    scheduler = get_linear_schedule_with_warmup(opt, 
                                                num_warmup_steps = 0, # Default value in run_glue.py
                                                num_training_steps = total_steps)
    bert_scheduler = get_linear_schedule_with_warmup(opt_bert, 
                                                num_warmup_steps = 0, # Default value in run_glue.py
                                                num_training_steps = total_steps)
    ...
    for epoch in range(args.tepoch):
        ...
        if args.do_train:
            if acc_lx_t > acc_lx_t_best:
                ...
                if args.woPRAM:
                    torch.save(model_bert,'best_WikiSQL_decoder_5_woPRAM.pt')
                elif args.woUR1:
                    torch.save(model_bert,'best_WikiSQL_decoder_5_woUR1.pt')
                elif args.worho:
                    torch.save(model_bert,'best_WikiSQL_decoder_5_worho.pt')
                elif args.freeze:
                    torch.save(model_bert,'best_WikiSQL_decoder_5_FB.pt')
                elif args.no_regular:
                    torch.save(model_bert,'best_WikiSQL_decoder_5_BERT.pt')
                else:
                    torch.save(model_bert,'best_WikiSQL_decoder_5.pt')
