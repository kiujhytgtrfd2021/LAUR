...
def get_bert_output_s2s(model_bert, tokenizer, nlu_t, hds, sql_vocab, max_seq_length,sample ):
    """
    ...
    for b, nlu_t1 in enumerate(nlu_t):
        ...
        # print(len(input_ids1),max_seq_length)
    ...
    if sample == None:
        outputs = model_bert(all_input_ids,all_input_mask ,all_segment_ids)
    else:
        outputs = model_bert(all_input_ids,all_input_mask ,all_segment_ids ,sample = sample)

    all_encoder_layer,pooled_output = outputs[2],outputs[1]
...
def get_bert_output(model_bert, tokenizer, nlu_t, hds, max_seq_length,sample):
    """
    ...
    for b, sql_i1 in enumerate(sql_i):
        ...
        if wn <= 0:
            ...
            for i_wn, conds1 in enumerate(conds):
                ...
                g_pnt_idxs1.append(st + 1 + end_pos_of_sql_vocab)  # due to inital [CLS] token in BERT-input vector？？？？？？？？ just beacause end_pos is [SEP] position
